<?php
/**
 * Reusable Content & Text Blocks Plugin by Defined Hosting - dh_cb_main Class
...
class dh_cb_main {
	static public $plugin_name = 'Reusable Content & Text Blocks by Defined Hosting';
	static public $plugin_version = '1.4.3';
	static public $plugin_wp_url = 'https://wordpress.org/plugins/definedhosting-content-blocks/';

	static public $option_name = 'dh_cb_option';
	static public $option_prefix = 'dh_cb_option_';
	static public $form_element_prefix = 'dh-cb-form-item-';
	static public $nonce_name = 'dh_cb_nonce';
	...
	static public function start( $plugin_file ) {
		...
		add_action( 'wp_ajax_dh_cb_mce_popup_form', __CLASS__ . '::mce_popup_form' );
		...
		add_action( 'wp_ajax_dh_cb_hide_notice', __CLASS__ . '::hide_notice' );
	...
	static public function register_post() {
		...
		register_post_type( self::$post_type_slug, array(
			'label'                => __( self::$post_type_labels, 'definedhosting-content-blocks-text-domain' ),
			'labels'               => array(
				'name'               => __( self::$post_type_labels, 'definedhosting-content-blocks-text-domain' ),
				'singular_name'      => __( self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'menu_name'          => __( self::$post_type_labels, 'definedhosting-content-blocks-text-domain' ),
				'name_admin_bar'     => __( self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'all_items'          => __( 'All ' . self::$post_type_labels, 'definedhosting-content-blocks-text-domain' ),
				'add_new'            => __( 'Add New ' . self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'add_new_item'       => __( 'Add New ' . self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'edit_item'          => __( 'Edit ' . self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'new_item'           => __( 'New ' . self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'view_item'          => __( 'View ' . self::$post_type_label, 'definedhosting-content-blocks-text-domain' ),
				'search_items'       => __( 'Search ' . self::$post_type_labels, 'definedhosting-content-blocks-text-domain' ),
				'not_found'          => __( 'No ' . strtolower( self::$post_type_labels ) . ' found', 'definedhosting-content-blocks-text-domain' ),
				'not_found_in_trash' => __( 'No ' . strtolower( self::$post_type_labels ) . ' found in Trash', 'definedhosting-content-blocks-text-domain' ),
				'parent_item_colon'  => __( 'Parent ' . self::$post_type_label, 'definedhosting-content-blocks-text-domain' )
			),
		...
		foreach ( $content_blocks as $content_block ) {
			if ( trim( $content_block->post_title ) == '' ) {
				$new_content_block_values = array(
					'ID'         => $content_block->ID,
					'post_title' => __( 'Content Block', 'definedhosting-content-blocks-text-domain' ) . ' ' . $content_block->ID
				);
	...
	static public function enqueue_scripts() {
		...
		wp_enqueue_script( 'dh_cb_admin_script', self::$plugin_url . 'js/admin.js', array(), self::$plugin_version );
		wp_enqueue_style( 'dh_cb_admin_style', self::$plugin_url . 'css/admin.css', array(), self::$plugin_version );
		...
		wp_add_inline_style( 'dh_cb_admin_style', $custom_css );

	}

	static public function admin_menu() {

		add_submenu_page( 'edit.php?post_type=' . self::$post_type_slug, __( 'Defined Hosting Content Blocks - Usage & About', 'definedhosting-content-blocks-text-domain' ), __( 'Usage & About', 'definedhosting-content-blocks-text-domain' ), 'edit_posts', self::$usage_about_page, __CLASS__ . '::usage_about_page' );
	...
	static public function add_action_links( $links ) {
		...
		$mylinks = array(
			'<a href="' . admin_url( 'edit.php?post_type=' . self::$post_type_slug . '&page=' . self::$usage_about_page ) . '">' . __( 'Usage & About', 'definedhosting-content-blocks-text-domain' ) . '</a>',
		);
	...
	static public function set_column_titles() {
		...
		return array(
			...
			'title'                             => __( 'Title', 'definedhosting-content-blocks-text-domain' ),
			self::$post_type_slug . 'shortcode' => __( 'Basic Shortcode', 'definedhosting-content-blocks-text-domain' ),
			'date'                              => __( 'Date', 'definedhosting-content-blocks-text-domain' )
		);
	...
	static public function set_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case self::$post_type_slug . 'shortcode' :
				...
				echo '<input type="text" class="dh_cb_read_only_input" value="' . esc_attr( '[' . self::$post_type_shortcode . ' id="' . $post_id . '"]' ) . '" style="width: 270px !important;"/><br />(' . __( 'Edit post for full list', 'definedhosting-content-blocks-text-domain' ) . ')';
				break;
	...
	static public function change_title_text( $title ) {
		...
		switch ( $post->post_type ) {
			case '' . self::$post_type_slug:
				$title = __( 'Enter content block name here', 'definedhosting-content-blocks-text-domain' );
				break;
		}
	...
	static public function mce_plugin( $plugin_array ) {
		if ( self::check_editor() ) {
			$plugin_array[ 'dh_cb_button' ] = self::$plugin_url . 'js/editor.js';
		}
	...
	static public function mce_button( $buttons ) {
		if ( self::check_editor() ) {
			array_push( $buttons, 'dh_cb_button' );
		}
	...
	static public function mce_popup_form() {
		if ( self::check_editor() ) {
			...
			echo '<div class="dh-cb-mce-form-container dh-cb-clearfix">';
			echo '<div id="dh-cb-mce-popup" class="hidden" style="max-width:800px">';
			echo '<div class="dh-cb-mce-field-container dh-cb-clearfix">';
			echo '<div class="dh-cb-mce-label"><label for="' . esc_attr( self::$option_prefix . 'cbid' ) . '">' . esc_html( __( 'Content Block:', 'definedhosting-content-blocks-text-domain' ) ) . '</label></div>';
			echo '<div class="dh-cb-mce-field dh-cb-clearfix"><select id="' . esc_attr( self::$option_prefix . 'cbid' ) . '" name="' . esc_attr( self::$option_prefix . 'cbid' ) . '" class="dh-cb-select-input" message="' . esc_attr( __( 'Content block must be selected', 'definedhosting-content-blocks-text-domain' ) ) . '">';
			echo '<option value="" selected="selected">' . esc_html( __( 'Select Content Block', 'definedhosting-content-blocks-text-domain' ) ) . '</option>';
			...
			echo '</div>';
			echo '<div class="dh-cb-mce-field-container dh-cb-clearfix">';
			echo '<div class="dh-cb-mce-label"><label for="' . esc_attr( self::$option_prefix . 'para' ) . '">' . esc_html( __( 'Content Filtering / Paragraph Tags:', 'definedhosting-content-blocks-text-domain' ) ) . '</label></div>';
			echo '<div class="dh-cb-mce-field dh-cb-clearfix"><select id="' . esc_attr( self::$option_prefix . 'para' ) . '" name="' . esc_attr( self::$option_prefix . 'para' ) . '" class="dh-cb-select-input">';
			foreach ( self::$para_list as $para_key => $para_value ) {
				if ( $para_key == 'none' ) {
					$para_key = '';
				}
				echo '<option value="' . esc_attr( $para_key ) . '"' . ( ( $para_key == '' ) ? ' selected="selected"' : '' ) . '>' . esc_html( __( $para_value, 'definedhosting-content-blocks-text-domain' ) ) . '</option>';
			}
			echo '</select></div>';
			echo '</div>';
			echo '<div class="dh-cb-mce-field-container dh-cb-clearfix">';
			echo '<div class="dh-cb-mce-field dh-cb-clearfix">';
			echo '<div class="dh-cb-mce-field-radio"><input type="radio" id="' . esc_attr( self::$option_prefix . 'use-1' ) . '" name="' . esc_attr( self::$option_prefix . 'use' ) . '" value="id" class="dh-cb-radio-input ' . esc_attr( self::$option_prefix . 'use' ) . '" checked="checked" /></div>';
			echo '<div class="dh-cb-mce-field-radio-label"><label for="' . esc_attr( self::$option_prefix . 'use-0' ) . '">' . esc_html( __( 'By ID', 'definedhosting-content-blocks-text-domain' ) ) . '</label></div>';
			echo '<div class="dh-cb-mce-field-radio"><input type="radio" id="' . esc_attr( self::$option_prefix . 'use-2' ) . '" name="' . esc_attr( self::$option_prefix . 'use' ) . '" value="slug" class="dh-cb-radio-input ' . esc_attr( self::$option_prefix . 'use' ) . '" /></div>';
			echo '<div class="dh-cb-mce-field-radio-label"><label for="' . esc_attr( self::$option_prefix . 'use-1' ) . '">' . esc_html( __( 'By Slug', 'definedhosting-content-blocks-text-domain' ) ) . '</label></div>';
			echo '</div>';
			echo '</div>';
			echo '<div class="dh-cb-mce-field-container dh-cb-clearfix">';
			echo '<div class="dh-cb-mce-field dh-cb-clearfix"><a href="#" id="' . esc_attr( self::$option_prefix . 'insert' ) . '" class="dh-cb-mce-button-blue">' . esc_html( __( 'Insert', 'definedhosting-content-blocks-text-domain' ) ) . '</a></div>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
			echo '</form>';
	...
	static public function meta_box( $post ) {
		add_meta_box( $post->id, __( 'Content Block Usage Codes', 'definedhosting-content-blocks-text-domain' ), __CLASS__ . '::meta_box_screen', self::$post_type_slug, 'normal', 'core' );
	}
	...
	static public function meta_box_screen( $post ) {
		...
		if ( self::is_edit_page( 'edit' ) ) {
			...
			$code[ 10 ] = '<?php echo dh_content_block_by_id( ' . $post_id . ' ); ?>';
			$code[ 11 ] = '<?php echo dh_content_block_by_id( ' . $post_id . ', \'no-shortcodes\' ); ?>';
			$code[ 12 ] = '<?php echo dh_content_block_by_id( ' . $post_id . ', \'paragraphs\' ); ?>';
			$code[ 13 ] = '<?php echo dh_content_block_by_id( ' . $post_id . ', \'paragraphs-no-shortcodes\' ); ?>';
			$code[ 14 ] = '<?php echo dh_content_block_by_id( ' . $post_id . ', \'full\' ); ?>';
			$code[ 15 ] = '<?php echo dh_content_block_by_slug( \'' . $post_slug . '\' ); ?>';
			$code[ 16 ] = '<?php echo dh_content_block_by_slug( \'' . $post_slug . '\', \'no-shortcodes\' ); ?>';
			$code[ 17 ] = '<?php echo dh_content_block_by_slug( \'' . $post_slug . '\', \'paragraphs\' ); ?>';
			$code[ 18 ] = '<?php echo dh_content_block_by_slug( \'' . $post_slug . '\', \'paragraphs-no-shortcodes\' ); ?>';
			$code[ 19 ] = '<?php echo dh_content_block_by_slug( \'' . $post_slug . '\', \'full\' ); ?>';
		} else {
			for ( $i = 0; $i <= 15; $i ++ ) {
				$code[ $i ] = __( 'New post', 'definedhosting-content-blocks-text-domain' );
			}
		}

		$access            = array(
			__( 'Shortcode', 'definedhosting-content-blocks-text-domain' ),
			__( 'PHP', 'definedhosting-content-blocks-text-domain' )
		);
		$methods           = array(
			__( 'ID', 'definedhosting-content-blocks-text-domain' ),
			__( 'Slug', 'definedhosting-content-blocks-text-domain' )
		);
		$para_descriptions = array(
			__( 'No Paragraph Tags / Run Shortcodes', 'definedhosting-content-blocks-text-domain' ),
			__( 'No Paragraph Tags / No Shortcodes', 'definedhosting-content-blocks-text-domain' ),
			__( 'Add Paragraph Tags / Run Shortcodes', 'definedhosting-content-blocks-text-domain' ),
			__( 'Add Paragraph Tags / No Shortcodes', 'definedhosting-content-blocks-text-domain' ),
			__( 'Full Content Filtering', 'definedhosting-content-blocks-text-domain' )
		);

		for ( $i = 0; $i <= 1; $i ++ ) {
			for ( $j = 0; $j <= 1; $j ++ ) {
				echo '<h1>' . esc_html( __( 'Via', 'definedhosting-content-blocks-text-domain' ) ) . ' ' . esc_html( $access[ $i ] ) . ' / ' . esc_html( __( 'By', 'definedhosting-content-blocks-text-domain' ) ) . ' ' . esc_html( $methods[ $j ] ) . '</h1>';
				echo '<div class="dh_cb_meta">';
				for ( $k = 0; $k <= 4; $k ++ ) {
					$index = ( $i * 10 ) + ( $j * 5 ) + $k;
					echo '<div class="dh_cb_meta_label"><label for="' . esc_attr( self::$option_prefix . 'access_' . $index ) . '">' . $para_descriptions[ $k ] . '</label></div><div class="dh_cb_meta_field"><input type="text" class="dh_cb_read_only_input" id="' . esc_attr( self::$option_prefix . 'access_' . $index ) . '" name="' . esc_attr( self::$option_prefix . 'access_' . $index ) . '" value="' . esc_attr( $code[ $index ] ) . '" /></div>';
				}
				echo '</div>';
			}
		}
	...
	static public function usage_about_page() {
		echo '<div class="wrap"><h1>' . esc_html( __( 'Usage & About', 'definedhosting-content-blocks-text-domain' ) ) . ' - ' . esc_html( __( self::$plugin_name, 'definedhosting-content-blocks-text-domain' ) ) . ' ' . esc_html( __( 'Version', 'definedhosting-content-blocks-text-domain' ) ) . ' ' . esc_html( __( self::$plugin_version, 'definedhosting-content-blocks-text-domain' ) ) . '</h1>';
		echo '<div class="dh-cb-admin-info-page-row dh-cb-clearfix">';
		echo '<div class="dh-cb-admin-info-page-column-half">';
		echo '<div class="dh-cb-admin-info-page-column-inner">';
		echo '<div class="postbox">';
		echo '<div class="inside">';
		echo '<div class="ls-bw-admin-info-page-column-content">';
		echo '<p>Defined Hosting&rsquo;s Reusable Content &amp; Text Blocks plugin allows you to define modular and repeated blocks of text and other content and place them within pages, posts, sidebars, widgetised areas or anywhere on your site via shortcodes, via the provided widget or via PHP.</p>';
		...
		echo '<p><strong>Note:</strong> The documentation on this page is also available with screenshots for guidance on the <a href="http://www.definedhosting.com/docs/reusable-content-text-blocks-wordpress-plugin/" target="_blank">Defined Hosting website</a>.</p>';
		...
		echo '<p>In order to do so, for the specific page builder, enable the &ldquo;dhcontentblock&rdquo; or &ldquo;Content Blocks&rdquo; post type within its settings. Also, when your content block contains content from a page builder, you must call (embed) the block with &ldquo;full content filtering&rdquo; or the PARA parameter set to &ldquo;full&rdquo;. <a href="#dh-cb-section-para">See section below</a> for full information regarding content processing and the PARA parameter.</p>';
		...
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block id="<span style="background-color: #ffff00; font-style: italic;">&lt;ID&gt;</span>"]</pre>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block slug="<span style="background-color: #ffff00; font-style: italic;">&lt;SLUG&gt;</span>"]</pre>';
		...
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block id="<span style="background-color: #ffff00; font-style: italic;">&lt;ID&gt;</span>" para="<span style="background-color: #ffff00; font-style: italic;">&lt;PARA&gt;</span>"]</pre>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block slug="<span style="background-color: #ffff00; font-style: italic;">&lt;SLUG&gt;</span>" para="<span style="background-color: #ffff00; font-style: italic;">&lt;PARA&gt;</span>"]</pre>';
		echo '<p><a href="#dh-cb-section-para">See section below</a> for full information regarding content processing and the PARA parameter.</p>';
		...
		echo '<p>The plugin provides a shortcode generator dialog box available from the WordPress TinyMCE editor window. To use it, place the cursor where you want the code to be inserted within the editor and click the Defined Hosting logo.</p>';
		...
		echo '<li style="padding-left: 30px; position: relative;"><span style="display: block; position: absolute; left: 10px;">•</span>The desired content processing (<a href="#dh-cb-section-para">See section below</a> for full information regarding content processing and the PARA parameter)</li>';
		...
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">&lt;? echo dh_content_block_by_id( <span style="background-color: #ffff00; font-style: italic;">&lt;ID&gt;</span> ); ?&gt;</pre>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">&lt;? echo dh_content_block_by_slug( \'<span style="background-color: #ffff00; font-style: italic;">&lt;SLUG&gt;</span>\' ); ?&gt;</pre>';
		echo '<p>You can also use the &ldquo;para&rdquo; parameter, which gives you greater control over how the content is filtered. For example:</p>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">&lt;? echo dh_content_block_by_id( <span style="background-color: #ffff00; font-style: italic;">&lt;ID&gt;</span>, \'<span style="background-color: #ffff00; font-style: italic;">&lt;PARA&gt;</span>\' ); ?&gt;</pre>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">&lt;? echo dh_content_block_by_slug( \'<span style="background-color: #ffff00; font-style: italic;">&lt;SLUG&gt;</span>\', \'<span style="background-color: #ffff00; font-style: italic;">&lt;PARA&gt;</span>\' ); ?&gt;</pre>';
		echo '<p><a href="#dh-cb-section-para">See section below</a> for full information regarding content processing and the PARA parameter.</p>';
		echo '<p id="dh-cb-section-para">An extensive list of the PHP code snippets for each content block can be found on the edit page for the specific block.</p>';
		...
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block getvar="<span style="background-color: #ffff00; font-style: italic;">&lt;VARIABLE NAME&gt;</span>"]</pre>';
		echo '<p><strong>Notes:</strong></p>';
		echo '<ul>';
		...
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block datetime="<span style="background-color: #ffff00; font-style: italic;">&lt;DATE/TIME FORMAT&gt;</span>"]</pre>';
		echo '<p>For example, the current date in dd/mm/yyyy format would be:</p>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block datetime="d/m/Y"]</pre>';
		...
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block info="site-title"]</pre>';
		echo '<p><strong>Current Page/Post Title:</strong></p>';
		echo '<pre style="margin-left: 40px; white-space: pre-wrap;">[dh_content_block info="page-title"]</pre>';
		...
		echo '<p>If you like this plugin ... or better still, if you <em><strong><span style="text-decoration: underline;">love</span></strong></em> it ... please take a moment to <a href="https://wordpress.org/support/plugin/definedhosting-content-blocks/reviews/" target="_blank">rate it on WordPress.org</a> and let the world know.</p>';
		echo '</div>';
		echo '</div>';
		echo '</div>';
		echo '</div>';
	...
	static public function hide_notice() {
		if ( isset( $_POST[ 'dh-cb-notice-hide' ] ) ) {
			self::update_option( 'notice-hide-' . trim( $_POST[ 'dh-cb-notice-hide' ] ), true );
			echo 'ok';
		}
	...
	static public function display_general_welcome() {
		...
		if ( ( time() - $first_time < 604800 ) && ( $impression_count < 200 ) ) { // Stop displaying after 7 days or 200 times
			if ( ! self::get_option( 'notice-hide-general-welcome' ) ) {
				$heading = esc_html( __( 'Welcome to version ' . self::$plugin_version . ' of ' . self::$plugin_name, 'definedhosting-content-blocks-text-domain' ) );
				if ( $current_screen->id == self::$post_type_slug . '_page_' . self::$usage_about_page ) {
					$message = esc_html( __( 'If you are new to the plugin, full usage instructions can be found below ... ', 'definedhosting-content-blocks-text-domain' ) ) . '<em>' . esc_html( __( 'Enjoy!', 'definedhosting-content-blocks-text-domain' ) ) . '</em>';
				} else {
		...
		if ( ( time() - $first_time < 604800 ) && ( $impression_count < 200 ) ) { // Stop displaying after 7 days or 200 times
			if ( ( $current_screen->post_type == self::$post_type_slug ) && ( ! self::get_option( 'notice-hide-make-donations' ) ) ) {
				$heading = esc_html( __( 'Make a donation', 'definedhosting-content-blocks-text-domain' ) );
				$message = esc_html( __( 'We develop, bugfix and support this plugin because we want to make it better and better and we always welcome any suggestions you offer. If it fits in with the core purpose of the plugin, we\'ll gladly consider it.', 'definedhosting-content-blocks-text-domain' ) ) . '<br /><br />' . esc_html( __( 'This plugin is provided free of charge, but we do welcome financial contributions ... ', 'definedhosting-content-blocks-text-domain' ) ) . '<a href="https://www.paypal.me/definedhosting" target="_blank">' . esc_html( __( 'Click here to make a donation', 'definedhosting-content-blocks-text-domain' ) ) . '</a>';
	...
	static public function display_notice( $notice_id, $heading, $message, $hideable, $show_branding = true ) {
		if ( ! self::get_option( 'notice-hide-' . $notice_id ) ) {
			echo '<div id="dh-cb-notice-' . esc_attr( $notice_id ) . '" class="dh-cb-notice notice is-dismissible"><div class="dh-cb-admin-info-page-column-content dh-cb-clearfix">';
			if ( $show_branding ) {
				echo '<div id="dh-cb-admin-branding-right"><a href="http://www.definedhosting.com/" target="_blank"><img src="' . esc_attr( self::$plugin_url ) . 'images/ls-wp-admin-branding.png" /></a></div>';
			}
			echo '<h2>' . $heading . '</h2><p>' . $message . '</p>';
			if ( $hideable ) {
				echo '<p class="dh-cb-notice-hide-container"><a id="dh-cb-notice-hide-' . esc_attr( $notice_id ) . '" class="dh-cb-notice-hide-link" href="#">' . esc_html( __( 'Hide this notice', 'definedhosting-content-blocks-text-domain' ) ) . '</a></p>';
			}
			echo '</div></div>';
		}
	}
